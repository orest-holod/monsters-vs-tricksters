...
GameEntity.prototype.getShadowColor = function () {

    return this._shadowColor;
}

GameEntity.prototype.setShadowColor = function (value) {
...
GameEntity.prototype.repaint = function () {
    ...
    if (this._IsNeedRepaint) {
        ...
        this._DOMElement.style.cssText = "".concat(
            ...
            "color: {color};".replace("{color}", this._color),
            "text-shadow: 0px 0px 3rem {color}".replace("{color}", this._shadowColor)
            );
...
function GameField(parentDOMElement) {
    ...
    this._starsLayer.setBackgroundPositionDY(0);
    this._cloudsLayer.setFPS(5);
    this._cloudsLayer.setBackgroundPositionDX(3);
}
...
GameFieldLayer.prototype.repaint = function () {
    ...
    if (this._isBackgroundPositionChanged) {
        ...
        this.getDOMElement().style.cssText = "background-position: {x}px {y}px".replace("{x}", this._backgroundPositionX).replace("{y}", this._backgroundPositionY);
...
function Game(parentDOMElement) {
    ...
    this._gameWrapper = document.getElementById("game-wrapper");
    ...
    this._gameFieldStarsLayer = this._gameField.getStarsLayer();
    this._gameFieldCloudsLayer = this._gameField.getCloudsLayer();
    this._gameFieldWaterLayer = this._gameField.getWaterLayer();

    this._tricksters = [];

    for (var i = 0; i < 26; i++) {

        var trickster = new Trickster(this._gameField.getDOMElement(), alphabet[i]);
...
Game.prototype.runGameLoop = function () {
    ...
    this._animationFrameManager.runAtFPS(function () {
        ...
        //var delta =  this._animationFrameManager.getSecondDelta();
        ...
        this._gameFieldStarsLayer.moveBackgroundX();
        this._gameFieldStarsLayer.moveBackgroundY();
        this._gameFieldCloudsLayer.moveBackgroundX();

        this._gameFieldWaterLayer.getDOMElement().innerHTML = this._animationFrameManager.getCurrentFPS() + "fps " + this._animationFrameManager.getSecondFPS() + "fps";
...
function AnimationFrameManager() {
    ...
    this._secondNow = Date.now();
    this._secondThen = this._secondNow;
    this._secondDelta = 0;
    this._secondAnimationFrameCount = 0;
    this._secondFPS = 60;

}

AnimationFrameManager.prototype.requestAnimationFrame = function (callback, fps) {

    this._now = Date.now();
    this._delta = this._now - this._then;
    this._currentFPS = Math.floor(1000 / this._delta);
    this._then = this._now;
...
AnimationFrameManager.prototype.getSecondFPS = function () {

    return this._secondFPS;
}

AnimationFrameManager.prototype.getSecondDelta = function () {

    return this._secondDelta;
}

AnimationFrameManager.prototype.runAtFPS = function (fn, fps) {

    this._secondNow = Date.now();
    this._secondDelta = this._secondNow - this._secondThen;
    this._secondFPS = Math.floor(1000 / this._secondDelta);

    if (this._secondDelta >= (1000 / fps) || this._currentFPS <= fps) {
        fn();
        this._secondThen = this._secondNow;
    }
